...
def ability_mod(self, score: int) -> int:
    # Typical B/X style: (simplified) 3-5:-3, 6-8:-1, 9-12:0, 13-15:+1, 16-17:+2, 18:+3
    if score <= 5:
        return -3
    if score <= 8:
        return -1
...
def roll_ability_check(self, ability: str, bonus: int = 0) -> RollResult:
    # Roll d20 + mod + bonus; DC adjudicated by GM
    r = roll("d20")
    total = r.total + bonus
    return RollResult(
...
def roll_attack(self, attack_bonus: int, damage: str) -> tuple[RollResult, RollResult]:
